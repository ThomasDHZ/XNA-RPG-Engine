...
namespace Test
{
    ...
    class TestMap : Microsoft.Xna.Framework.Game
    {
        ...
        String Map;
        public TestMap(Texture2D textures, bool Collidable)
        {
            tile = new Tile[10];
        ...
        public void LoadMap(string mapname)
        {
            ...
            for (int x = 0; x < XArea; x++)
            {
                ...
                {
                   /* if (XArea <= 17 && YArea <= 11)
                    {
                        if (XArea >= 9 && YArea >= 6)
                        {
                            int startx = TILESIZE * (XArea / 2);
                            int starty = TILESIZE * (XArea / 2);
                            TileMap[x][y] = tile[numbers[x][y]];
                            TileMap[x][y].SetPositionX(startx + (x * TILESIZE));
                            TileMap[x][y].SetPositionY(starty + (y * TILESIZE));
                        }
                        else
                        {
                            int startx = TILESIZE * (XArea / 2);
                            TileMap[x][y] = tile[numbers[x][y]];
                            TileMap[x][y].SetPositionX(startx - (x * TILESIZE));
                            TileMap[x][y].SetPositionY((y * TILESIZE));
                        }
                    }
                    else
                    {*/
                        TileMap[x][y] = tile[numbers[x][y]];
                        TileMap[x][y].SetPositionX(x * TILESIZE);
                        TileMap[x][y].SetPositionY(y * TILESIZE);
                    //}
                    int adsf = 34;
                }
